...
func findIntersectionSteps(wire1 []coord, wire2 []coord) []int {
	var overlap []int
	for i, wire1Pt := range wire1 {
		for j, wire2Pt := range wire2 {
			if wire1Pt == wire2Pt {
				overlap = append(overlap, i+j+2)
			}
		}
	}
	return overlap
...
func findShortest(points []int) int {
	shortestDist := points[0]
...
func main() {
	...
	fmt.Println(findShortest(findIntersectionSteps(wire1Coords, wire2Coords)))
}
