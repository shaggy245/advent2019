...
func createCoordinates(wire []string, start coord) []coord {
	/* Using Taxicab geometry and given a list of directions, return all
	points on a grid that the directions touch
	*/
	coords := make([]coord, 1)
	coords[0] = start
	for _, move := range wire {
		dir := string(move[0])
		qty, _ := strconv.Atoi(string(move[1:]))
		switch dir {
		case "R":
			for j := 1; j <= qty; j++ {
				coords = append(coords, coord{x: coords[len(coords)-1].x + 1, y: coords[len(coords)-1].y})
			}
		case "L":
			for j := 1; j <= qty; j++ {
				coords = append(coords, coord{x: coords[len(coords)-1].x - 1, y: coords[len(coords)-1].y})
		...
		case "U":
			for j := 1; j <= qty; j++ {
				coords = append(coords, coord{x: coords[len(coords)-1].x, y: coords[len(coords)-1].y + 1})
			}
		case "D":
			for j := 1; j <= qty; j++ {
				coords = append(coords, coord{x: coords[len(coords)-1].x, y: coords[len(coords)-1].y - 1})
...
func findIntersection(wire1 []coord, wire2 []coord) []coord {
	var overlap []coord
	for _, wire1Pt := range wire1 {
		for _, wire2Pt := range wire2 {
			if wire1Pt == wire2Pt {
				overlap = append(overlap, wire1Pt)
			}
		}
	}
	return overlap
...
func findClosest(points []coord, origin coord) float64 {
	closestDist := math.Abs(float64((points[0].x - origin.x) + (points[0].y - origin.y)))
	for _, pt := range points {
		dist := math.Abs(float64((pt.x - origin.x) + (pt.y - origin.y)))
		if dist < closestDist {
			closestDist = dist
		}
	}
